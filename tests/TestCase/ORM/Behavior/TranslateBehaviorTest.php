...
class TranslateBehaviorTest extends TestCase
{
    ...
    public function testTranslationsHasMany()
    {
        ...
        $results = $table->find('translations')->contain([
            'Comments' => function ($q) {
                $q->find('translations')->select(['id', 'comment', 'article_id']);

                return $q;
            },
        ]);

        $stmt = ConnectionManager::get('default')->query('SELECT * FROM articles');
        debug($stmt->fetchAll('assoc'));
        $stmt->closeCursor();

        $stmt = ConnectionManager::get('default')->query('SELECT * FROM comments');
        debug($stmt->fetchAll('assoc'));
        $stmt->closeCursor();

        $stmt = ConnectionManager::get('default')->query('SELECT * FROM i18n');
        debug($stmt->fetchAll('assoc'));
        $stmt->closeCursor();
